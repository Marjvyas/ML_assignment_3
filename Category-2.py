...
st.sidebar.write("Model-2: Context=5, Embedding=32, Activation=Tanh")
...
MODEL_FILENAME_MAP = {
    (3, 64, "ReLU"): "best_model (2).pth",
    (5, 32, "Tanh"): "best_model (3).pth",
}
...
def main():
    ...
    st.title("Next Word Prediction — Category-2")
    ...
    if LOAD_BUTTON:
        ...
        if model_path is None:
            ...
            try:
                with open("stoi (1).json", "rb") as f:
                    stoi_bytes = f.read()
                with open("itos (1).json", "rb") as f:
                    itos_bytes = f.read()
